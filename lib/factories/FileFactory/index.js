...
const buildPaths = (path, inputPath, relativeOutputPath) => {
...
class FileFactory {
    ...
    async _extractFileData() {
        return await this._fileBuilder.extractFileData(this._filePaths.inputPath, this._options);
    }

    async _transformFileData(extractedData) {
        return await this._fileBuilder.transformFileData(this._filePaths, this._options, extractedData);
    }

    async _createFile(data) {
        return await this._fileBuilder.createFile(this._filePaths, data);;
    }
    ...
    async _buildDependenciesList(data) {
        ...
        return [].concat(
            paths.map(({ path }) => buildPaths(path, inputPath, relativeOutputPath)),
            plugins.map(({ path }) => buildPaths(path, inputPath, relativeOutputPath))
        );
    }
