...
const handleImportsDeclarationType = (variable, path, codeDefinition) => {
	if (!path) {
		return '';
	};
	//codeDefinition.indexOf(variable) < 0
	if (variable === UNSET_IMPORT_VAR || !regexHelper.variableIsUsed(variable, codeDefinition)) {
		return `import '${path}';`;
	};
...
const createImportsDeclaration = ({ variable, path, rootRelative }, paths, codeDefinition) => {
	if (!rootRelative) {
		return handleImportsDeclarationType(variable, path, codeDefinition);
	};
	...
	return handleImportsDeclarationType(variable, relativePath.replace(ext, ''), codeDefinition);
};
