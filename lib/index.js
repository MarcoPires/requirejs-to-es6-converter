...
/*
 - ex:
node ./lib/index.js configFile=./config deleteMigrationDir=true
node ./lib/index.js entryFilePath=H:/leap/leap-ui/dev_online/js/library/controllers/CatalogEntriesController.js migrationPath=H:/ProjectY/ui-legacy-publications-library/src/common/legacy configFile=./config
 */
const defaults = () => ({
...
const buildArgs = async (cwd, args) => {
	...
	return await ArgumentsValidator.validate(extendedArgs, {
		'entryFilePath': {
			checkPath: true/* ,
			contains: 'ui-building-helper' */
		},
		'projectRootDir': {},
		'migrationPath': {
			checkPath: true,
			removeDir: (!!extendedArgs.deleteMigrationDir || false),
			mkDir: true
		}
	});
};
...
class ProjectMigration {
	...
	_processDependencies(dependencies = []) {
		...
		dependencies.map(dependency => {
			...
			const filePaths = this._buildFilePaths(dependency);
